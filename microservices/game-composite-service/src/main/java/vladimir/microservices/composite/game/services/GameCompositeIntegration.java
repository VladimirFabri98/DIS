...
public class GameCompositeIntegration implements GameService, ReviewService, DlcService, GameEventService {
	...
	private final int gameServiceTimeoutSec;

	// Ove vrednosti treba da se poklapaju sa nazivima kontejnera u docker-compose
	// fajlu
	...
	public GameCompositeIntegration(WebClient.Builder builder, ObjectMapper mapper, MessageSources messageSources,
			@Value("${app.game-service.timeoutSec}") int gameServiceTimeoutSec) {

		this.webClientBuilder = builder;
		this.mapper = mapper;
		this.messageSources = messageSources;
		this.gameServiceTimeoutSec = gameServiceTimeoutSec;
	}

	//@Retry(name="game")
	@CircuitBreaker(name="game")
	@Override
	public Mono<Game> getGame(int gameId, int delay, int faultPercent) {

		// http://game/1 ?????
		URI url = UriComponentsBuilder
				.fromUriString(gameServiceUrl + "/game/{gameId}?delay={delay}&faultPercent={faultPercent}")
				.build(gameId, delay, faultPercent);
	...
	public Flux<Review> getReviews(int gameId) {
		URI url = UriComponentsBuilder
				.fromUriString(reviewServiceUrl + "/review/{gameId}")
				.build(gameId);
	...
	public Flux<Dlc> getDlcs(int gameId) {
		URI url = UriComponentsBuilder
				.fromUriString(dlcServiceUrl + "/dlc/{gameId}")
				.build(gameId);
	...
	public Flux<GameEvent> getEvents(int gameId) {
		...
		URI url = UriComponentsBuilder
				.fromUriString(eventServiceUrl + "/game-event/{gameId}")
				.build(gameId);
		//String url = eventServiceUrl + "/" + gameId;
		LOG.debug("Will call getEvents API on URL: {}", url);
		return getWebClient().get().uri(url).retrieve().bodyToFlux(GameEvent.class).log()
	...
	public GameEvent createEvent(GameEvent body) {
		messageSources.outputGameEvents()
				.send(MessageBuilder
						.withPayload(new Event<Integer, GameEvent>(CREATE, body.getGameId(), body, LocalDateTime.now()))
						.build());
		return body;
	}
	...
	public void deleteDlcs(int gameId) {
		messageSources.outputDlcs().send(
				MessageBuilder.withPayload(new Event<Integer, Dlc>(DELETE, gameId, null, LocalDateTime.now())).build());
	}
